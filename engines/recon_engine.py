...
def start_recon_sequence(target, full_scan=False, throttle=0.01, grab_banner=True, thread_count=150):
    ...
    if full_scan:
        ports = range(1, 65536)
        results.append(f"[*] Full scan mode — 65,535 ports")
    else:
        ports = TOP_1000
        results.append(f"[*] Standard scan — top {len(TOP_1000)} ports")
    ...
    threads = []
    for _ in range(min(thread_count, port_queue.qsize())):
        t = threading.Thread(
            target=threader,
            args=(target, port_queue, results, throttle, grab_banner),
            daemon=True
        )
        threads.append(t)
        t.start()
    ...
    if _stop_event.is_set():
        results.append("[!] SCAN TERMINATED BY USER")
    elif not any("[+]" in r for r in results):
        results.append("[-] No open ports found in scanned range.")

    # Sort: info lines first, then open ports by port number
    info_lines = [r for r in results if "[*]" in r or "[-]" in r or "[!]" in r]
    open_lines = sorted(
        [r for r in results if "[+]" in r],
        key=lambda x: int(x.split()[1].split("|")[0].strip()) if x.split()[1].split("|")[0].strip().isdigit() else 0
    )

    return info_lines + open_lines
